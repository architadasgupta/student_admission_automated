...
def generate_application_pdf(student_id):
    """Generate a PDF application report for a student"""
    try:
        ...
        # Parse storage data if it exists and is a string
        if "storage_data" in metadata and isinstance(metadata["storage_data"], str):
            try:
                parsed_data = json.loads(metadata["storage_data"])
                # Don't completely override metadata, as it might contain other fields
                # that aren't in storage_data
                for key, value in parsed_data.items():
                    metadata[key] = value
            except json.JSONDecodeError:
                print(f"Could not parse stored application data for {student_id}")
        
        # Debug logging to check data structure
        print(f"Generating PDF for student: {student_id}")
        print(f"Feedback data: {metadata.get('status', {}).get('feedback', 'No feedback data')}")
        
        # Create PDF document
        pdf_buffer = create_pdf_document(student_id, metadata)
        
        # Save PDF to disk for future reference
        pdf_dir = os.path.join(os.getcwd(), "feedback_pdfs")
        os.makedirs(pdf_dir, exist_ok=True)  # Create directory if it doesn't exist
        
        pdf_filename = f"{student_id}_application_report.pdf"
        pdf_path = os.path.join(pdf_dir, pdf_filename)
        
        with open(pdf_path, "wb") as f:
            f.write(pdf_buffer.getvalue())
        
        # Return both the buffer and the path
        return {
            "buffer": pdf_buffer,
            "path": pdf_path,
...
def safely_get_data(data, path, default=None):
    """
    Safely extract nested data from a dictionary with error handling and json string parsing
    ...
    current = data
    for key in path:
        # Handle case where current is a string that might be JSON
...
def create_pdf_document(student_id, data):
    ...
    # Add student information (personal and academic data extraction remains unchanged)
    ...
    # Feedback Section - Status-Aware Logic
    feedback_data = safely_get_data(data, ['status', 'feedback'], {})
    
    if application_status == "Submitted-Accepted":
        elements.append(Paragraph("Congratulations on your successful application!", styles['NormalBold']))
        
        # Add specific feedback for accepted applications
        rank = safely_get_data(data, ['academic', 'exam_rank'], "N/A")
        percentage = safely_get_data(data, ['academic', 'board_percentage'], "N/A")
        
        elements.append(Paragraph(f"Your WBJEE rank of {rank} and class 12th percentage of {percentage}% were instrumental in securing your admission.", styles['Normal']))
        
        # Loan details if applicable
        loan_status = safely_get_data(data, ['loan', 'status', 'status'], "Not Applied")
        if loan_status == "Approved":
            loan_amount = safely_get_data(data, ['loan', 'status', 'approved_amount'], 0)
            elements.append(Paragraph(f"Loan Status: Approved. Approved Amount: ₹{loan_amount:,}.", styles['Normal']))
            elements.append(Paragraph("Please complete the financial documentation at [link].", styles['Normal']))
        
        elif loan_status == "Pending":
            elements.append(Paragraph("Loan Status: Pending. Please submit additional financial documents to finalize the loan.", styles['Normal']))
        
    elif application_status == "Rejected":
        elements.append(Paragraph("We regret to inform you that your application was not successful.", styles['NormalBold']))
        
        # Add improvement suggestions for rejected applications
        ...
        elements.append(Paragraph("Your application is currently under review.", styles['NormalBold']))
        
        # Provide generic suggestions for pending applications
        suggestions = [
            "Ensure all required documents are submitted.",
            "Check your registered email for updates on your application."
        ]
        
        elements.append(Paragraph("Suggestions:", styles['NormalBold']))
...
def get_pdf_download_link(pdf_buffer, filename="application_report.pdf"):
    ...
    if isinstance(pdf_buffer, dict) and 'buffer' in pdf_buffer:
        if hasattr(pdf_buffer['buffer'], 'getvalue'):
            b64_pdf = base64.b64encode(pdf_buffer['buffer'].getvalue()).decode('utf-8')
        else:
            return None
    # Handle BytesIO case
    elif hasattr(pdf_buffer, 'getvalue'):
...
def display_pdf(pdf_buffer):
    ...
    if isinstance(pdf_buffer, dict) and 'buffer' in pdf_buffer:
        if hasattr(pdf_buffer['buffer'], 'getvalue'):
            base64_pdf = base64.b64encode(pdf_buffer['buffer'].getvalue()).decode('utf-8')
        else:
            return None
    # Handle BytesIO case    
    elif hasattr(pdf_buffer, 'getvalue'):
    ...
    # Create iframe with correct content type and styling
    pdf_display = f'''
...
def check_application_status(student_id):
    """Display application status and PDF in Streamlit interface"""
    ...
    # Parse storage data if needed
    if "storage_data" in metadata and isinstance(metadata["storage_data"], str):
        try:
            parsed_data = json.loads(metadata["storage_data"])
            # Update metadata with parsed data for display
            for key, value in parsed_data.items():
    ...
    # Check if PDF already exists or generate a new one
    pdf_dir = os.path.join(os.getcwd(), "feedback_pdfs")
    os.makedirs(pdf_dir, exist_ok=True)  # Create directory if it doesn't exist
    pdf_filename = f"{student_id}_application_report.pdf"
    pdf_path = os.path.join(pdf_dir, pdf_filename)
    ...
    if os.path.exists(pdf_path):
        ...
        try:
            ...
            st.error(f"Error loading existing PDF: {str(e)}")
            # Fall back to generating a new one
            pdf_buffer = None
    
    if pdf_buffer is None:
        # Generate PDF if it doesn't exist or couldn't be loaded
        pdf_result = generate_application_pdf(student_id)
        if pdf_result and isinstance(pdf_result, dict) and 'buffer' in pdf_result:
            pdf_buffer = pdf_result["buffer"]
        else:
            st.error("Could not generate application report")
            return
    
    # Display application information - simplified to avoid repetition
    ...
    with col2:
        st.write(f"**Category:** {personal.get('category', 'N/A')}")
        st.write(f"**Date of Birth:** {personal.get('dob', 'N/A')}")
        st.write(f"**Gender:** {personal.get('gender', 'N/A')}")
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.write(f"**Exam Type:** {academic.get('exam_type', 'N/A')}")
        st.write(f"**Exam Rank:** {academic.get('exam_rank', 'N/A')}")
        st.write(f"**Board Percentage:** {academic.get('board_percentage', 'N/A')}%")
    with col2:
        st.write(f"**Physics Marks:** {academic.get('physics_marks', 'N/A')}")
        st.write(f"**Chemistry Marks:** {academic.get('chemistry_marks', 'N/A')}")
        st.write(f"**Mathematics Marks:** {academic.get('maths_marks', 'N/A')}")
    
    # Status information
    status = metadata.get("status", {})
    st.subheader("Application Status")
    
    # Show overall status with appropriate color
    application_status = status.get("application_status", "Submitted")
    status_color = {
        "Accepted": "green",
        "Rejected - Not Eligible": "red",
        "Rejected - Not Shortlisted": "red",
        "Processed": "blue",
        "Submitted": "orange"
    }.get(application_status, "gray")
    
    st.markdown(f"<h3 style='color:{status_color};'>Status: {application_status}</h3>", unsafe_allow_html=True)
    
    # Show detailed statuses
    col1, col2, col3, col4 = st.columns(4)
    
    with col1:
        doc_status = safely_get_data(status, ["document_verification", "status"], "Pending")
        st.write(f"**Documents:** {doc_status}")
    
    with col2:
        elig_status = safely_get_data(status, ["eligibility", "status"], "Pending")
        st.write(f"**Eligibility:** {elig_status}")
    
    with col3:
        short_status = safely_get_data(status, ["shortlisting", "status"], "Pending")
        st.write(f"**Shortlisting:** {short_status}")
    ...
    with col4:
        ...
        if loan_required:
            ...
            st.write(f"**Loan:** {loan_status}")
        else:
            st.write("**Loan:** Not Applied")
    
    # Display feedback from the feedback agent if available
    feedback_data = safely_get_data(status, ["feedback"], {})
    ...
    st.write("Your detailed application report is available below:")
    
    col1, col2 = st.columns([3, 1])
    
    with col1:
        # Display PDF within Streamlit
        if pdf_buffer:
            pdf_display = display_pdf(pdf_buffer)
            if pdf_display:
    ...
    with col2:
        ...
        if pdf_buffer:
            download_link = get_pdf_download_link(pdf_buffer, pdf_filename)
            if download_link:
            ...
            if st.button("Regenerate PDF Report"):
                pdf_result = generate_application_pdf(student_id)
                if pdf_result and isinstance(pdf_result, dict) and 'buffer' in pdf_result:
                    st.success("PDF report has been regenerated successfully!")
                    st.experimental_rerun()  # Refresh the page to show new PDF
                else:
                    st.error("Failed to regenerate PDF report")
...
def process_application(student_id, student_data, marksheet_text, scorecard_text):
    ...
    try:
        ...
        feedback_agent = Agent(
            ...
            goal=("Provide concise, non-redundant feedback tailored to the application outcome"),
            backstory=("Specializes in analyzing application results and providing focused guidance"),
            tools=[file_writer_tool],
            constraints=[
                "Do not repeat information that appears in other sections of the report",
                "For accepted applications, focus on next steps rather than improvements",
                "For rejected applications, be specific about reasons without repeating eligibility criteria",
                "Never include boilerplate text that doesn't directly address the student's situation",
            ]
        )
        
        pdf_agent = Agent(
            role="PDF Generation Agent",
            goal=("Generate comprehensive PDF reports of application outcomes and save them to a dedicated folder"),
            backstory=("Expert in creating professional, well-formatted documents with all relevant application information and organizing them for easy access")
        )
        ...
        feedback_task = Task(
            description=(
                f"Generate personalized feedback for student {student_id} based on their application outcome. "
                f"For ACCEPTED applications, focus on: "
                f"- Next steps for enrollment "
                f"- Preparation suggestions "
                f"- Available resources "
                f"For REJECTED applications, provide: "
                f"- Specific reasons for rejection "
                f"- Clear improvement areas "
                f"- Alternative paths "
                f"Tailor all feedback to the student's specific academic profile and status."
            ),
            expected_output=(
                "Concise, actionable feedback appropriate for the application outcome "
                "with no redundant information across sections"
            ),
        ...
        pdf_generation_task = Task(
            description=(
                f"For student {student_id}, organize all application data, status details, and feedback "
                f"into a structured JSON format that could be used to generate a PDF report. "
                f"Include sections for personal information, academic details, application status, "
                f"eligibility assessment, shortlisting details, and any available feedback."
            ),
            expected_output=(
                "A structured JSON dictionary with all the information of the applicants about the status updates organized into sections "
                "that would be included in a PDF report."
            ),
            agent=pdf_agent,
            dependencies=[eligibility_task, shortlisting_task]
            )
        ...
        elif not shortlisting_result.get('is_shortlisted', False):
            updated_data['status']['application_status'] = "Rejected - Not Shortlisted"
        elif loan_result.get('loan_approved', False):
            updated_data['status']['application_status'] = "Accepted - Pending Financial Clearance"
        else:
            updated_data['status']['application_status'] = "Accepted"
        
        # Update Pinecone record
        try:
            ...
            if current_embedding is not None:
                # Create a metadata dict with properly flattened fields for Pinecone
                metadata = {}
                
                # Add personal info directly
                for key, value in updated_data.get('personal', {}).items():
                    if isinstance(value, (str, int, float, bool)):
                        metadata[f"personal_{key}"] = value
                
                # Add academic info directly
                for key, value in updated_data.get('academic', {}).items():
                    if isinstance(value, (str, int, float, bool)):
                        metadata[f"academic_{key}"] = value
                
                # Add status info - simplified to avoid nesting issues
                metadata["application_status"] = updated_data.get('status', {}).get('application_status', "Submitted")
                
                if 'eligibility' in updated_data.get('status', {}):
                    metadata["eligibility_status"] = updated_data['status']['eligibility'].get('status', "Pending")
                
                if 'shortlisting' in updated_data.get('status', {}):
                    metadata["shortlisting_status"] = updated_data['status']['shortlisting'].get('status', "Pending")
                
                # Add loan info directly
                if updated_data.get('loan', {}).get('required', False):
                    metadata["loan_required"] = True
                    metadata["loan_status"] = updated_data.get('loan', {}).get('status', {}).get('status', "Pending")
                else:
                    metadata["loan_required"] = False
                
                # Store ID and name for easy access
                metadata["id"] = student_id
                metadata["full_name"] = updated_data.get('personal', {}).get('name', "Unknown")
                
                # Store the complete data as a JSON string
                try:
                    metadata["storage_data"] = json.dumps(updated_data)
                except json.JSONDecodeError:
                    st.warning("Could not serialize full data")
                ...
                # Generate PDF right after updating data
                pdf_result = generate_application_pdf(student_id)
                if pdf_result:
                    st.success("Application PDF report generated successfully.")
                else:
                    st.warning("Could not generate PDF report.")
            else:
                st.warning("Could not update database due to embedding issues.")
    ...
    if check_status or ('last_checked_id' in st.session_state and st.session_state['last_checked_id'] == application_id):
        ...
        if application_data:
            try:
                ...
                with status_cols[0]:
                    doc_status = safely_get_data(status_data, ['document_verification', 'status'], "Processed")
                    st.text(f"Documents: {doc_status}")
                
                with status_cols[1]:
                    elig_status = safely_get_data(status_data, ['eligibility', 'status'], "Processed")
                    st.text(f"Eligibility: {elig_status}")
                
                with status_cols[2]:
                    short_status = safely_get_data(status_data, ['shortlisting', 'status'], "Processed")
                ...
                pdf_result = generate_application_pdf(application_id)
                if pdf_result and isinstance(pdf_result, dict) and 'buffer' in pdf_result:
                    pdf_buffer = pdf_result['buffer']
                else:
                    pdf_buffer = None
                ...
                if pdf_buffer:
                    ...
                    # Helper function to standardize buffer handling
                    def ensure_bytesio_buffer(pdf_data):
                        if isinstance(pdf_data, io.BytesIO):
                            return pdf_data
                        elif isinstance(pdf_data, dict) and 'buffer' in pdf_data and isinstance(pdf_data['buffer'], io.BytesIO):
                            return pdf_data['buffer']
                        return None
                    
                    # Make sure we have a proper BytesIO buffer
                    processed_buffer = ensure_bytesio_buffer(pdf_buffer)
                    ...
                    if processed_buffer:
                        ...
                        with pdf_tabs[0]:
                            # Make sure to reset buffer position before display
                            processed_buffer.seek(0)
                            pdf_html = display_pdf(processed_buffer)
                            if pdf_html:
                        ...
                        with pdf_tabs[1]:
                            ...
                            processed_buffer.seek(0)
                            download_link = get_pdf_download_link(processed_buffer, f"{application_id}_application_report.pdf")
                            if download_link:
                                st.markdown(download_link, unsafe_allow_html=True)
                        ...
                        st.error("Could not process PDF buffer. Try again later.")
                else:
